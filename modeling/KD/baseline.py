...
class Scratch(nn.Module):
    ...
    def score_mat_to_save(self):
        with torch.no_grad():
            return self.backbone.score_mat().cpu()
...
class DCD(BaseKD4Rec):
    def __init__(self, args, teacher, student):
        ...
        self.T_rank = torch.arange(self.mxK).repeat(self.num_users, 1).cuda()
    ...
    def do_something_in_each_epoch(self, epoch):
        with torch.no_grad():
            S_pred = self.student.get_all_ratings()
            S_topk = torch.argsort(S_pred, descending=True, dim=-1)
            S_rank = torch.argsort(S_topk, dim=-1)
